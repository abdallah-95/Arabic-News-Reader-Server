...
public class RootStemmer {
    ...
    public String stem(String text){
    	...
    	for(int i=0;i<tokens.length;i++){
    		...
    		stemmedDocument[wordNumber][0] = tokens[i];
    		
    		String currentWord=rs.findRoot(tokens[i]);
    		text=text+currentWord+" ";
    		
    		if ( stemmedDocument[wordNumber][2] == null )
            {
    	        Vector punctuations = ( Vector ) staticFiles.elementAt ( 11 );
    	        Vector stopwords = ( Vector ) staticFiles.elementAt ( 13 );
    			
    	        if(stopwords.contains(currentWord)){
    	        	 stemmedDocument[wordNumber][1] = currentWord;
    	             stemmedDocument[wordNumber][2] = "STOPWORD";
    			}
    	        else if(punctuations.contains(currentWord)){
    	        	stemmedDocument[wordNumber][1] = currentWord; 
    	        	stemmedDocument[wordNumber][2] = "PUNCTUATION";
    	        }
    	        else{
    			stemmedDocument[wordNumber][1] = tokens[i];
                stemmedDocument[wordNumber][2] = "NOT STEMMED";
    	        }
            }
    		
    		
    		text2=text2+stemmedDocument[wordNumber][0]+"\t"+
    		            stemmedDocument[wordNumber][1]+"\t"+
    		            stemmedDocument[wordNumber][2]+"\n";
    ...
    private String findRoot ( String token )
    {   
    	rootFound = false;
        stopwordFound = false;
        fromSuffixes = false;
